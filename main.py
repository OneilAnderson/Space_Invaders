...
class Ship:
    ...
    def draw(self,window):
        # pygame.draw.rect(window, (255,0,0), (self.x,self.y, 50, 50)) Image rectangle example
...
class Player(Ship):
    def __init__(self, x, y, health=100):
        super().__init__(x, y, health) # initializes player in original class and allthe methods
...
def main():
    ...
    player_vel = 5 # player moevement speed
    laser_vel = 5
    ...
    while run:
        ...
        if keys[pygame.K_UP] and player.y - player_vel > 0: # move up
            player.y -= player_vel
        if keys[pygame.K_DOWN] and player.y + player_vel + player.get_height() + 20 < HEIGHT: # move down
            player.y += player_vel
